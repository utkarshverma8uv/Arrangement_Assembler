...
try:
    myco= mysql.connect(host='localhost',user='rot',password= pas)
    if myco.is_connected()==True:
        ...
        try:
            ...
            cursor=mycon.cursor()
            try:
                cursor.execute('create table mat(Day varchar(10), 1st int, 2nd int,3rd int,4th int,5th int,6th int,7th int,8th int)')    
                cursor.execute("insert into mat values('mon',6,7,8,8,null,9,null,10)")
                cursor.execute("insert into mat values('tue',6,7,8,8,null,9,null,10)")
                cursor.execute("insert into mat values('wed',6,7,8,8,null,9,null,10)")
                cursor.execute("insert into mat values('thu',10,9,9,null,8,6,7,null)")
                cursor.execute("insert into mat values('fri',10,9,9,null,8,6,7,null)")
                cursor.execute("insert into mat values('sat',10,9,9,null,8,6,7,null)")
                mycon.commit()
                print('mat created')
            except:
